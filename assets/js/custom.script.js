...
(function($) {
    ...
    $(document).ready(function () {
        $.getJSON("/data/data.json", function (data) {
         ...
         //  var imageGallary = "";
          ...
          $.each(data.schedule, function (key,value){
            scheduleHeaders += `<li>
                                 <a data-toggle="pill" href="#menu${value.menu}">
                                    <h3>${value.dayName}</h3>
                                    <p><span>${value.date} </span>${value.month}, ${value.year}</p>
                                 </a>
                              </li>`;
             schedule += ` <div id="menu${value.menu}" class="tab-pane fade in ${value.details.status}">
                                 <div
                                    class="panel-group"
                                    id="accordion2"
                                    role="tablist"
                                          ...
                                          <div class="panel-title">
                                             <a
                                                ...
                                                <div class="lgx-single-schedule">
                                                   ${value.details.author.bool === true ?  `<div class="author">
                                                      <img src=${value.details.author.imageSrc} alt=${value.details.author.authName}/></div>`:""}
                                                   
                                                   <div class="schedule-info">
                                                      <h4 class="time">
                                                         ${value.date} <span>${value.month}</span>
                                                      </h4>
                                                      <h3 class="title">
                                                         ${value.details.title}
                                                      </h3>
                                                      <h4 class="author-info">
                                                         Secured By <span>${value.details.author.authName}</span>,
                                                         ${value.details.author.authInfo}
                                                      </h4>
                                                   </div>
                                                </div>
                                             </a>
                                       ...
                                       <div
                                          ...
                                          <div class="panel-body">
                                             <p class="text">
                                                ...
                                                ${value.details.extra.map(m => {
                                                   return `<h4 class="location">
                                                <strong>${m.title}:</strong>
                                                <a href=${m.titleSrc} tar get="_blank"
                                                   >${m.titleSrc}</a>
                                             </h4>`
                                                }).join('')}
          ...
          $.each(data.events, function (key, value) {
            eventdata += `<div class="col-xs-12 col-sm-6 col-md-4">
          ...
          $.each(data.gaming, function (key, value) {
            gamingdata += `<div class="col-xs-12 col-sm-6 col-md-4">
                  <div class="lgx-single-speaker2 lgx-single-speaker3">
                    <!--lgx-single-speaker-circle lgx-single-speaker2 lgx-single-speaker3-->
                    <figure>
                      <figcaption>
         ...
         //  $.each(data.imageGallary, function (key, value) {
         //    imageGallary += `<div class="item-gallery">
         //                   <figure style="height: 660px; width: 100%">
         //                      <img loading="lazy" src=${value.gallarySrc} alt=${value.gallaryName} />
         //                   </figure>
         //                </div>`
         //  });
        ...
        $.each(data.notification,function(key,value){
         notification += `<h4>${greetings} folks,${today.getDate() === value.eventDay && today.getHours() >= value.eventHour && today.getMinutes() >= value.eventMinutes ? " Enjoy the Tech Fest": ` ${value.eventName} is going on in room no. ${value.roomNumber}`} </h4>
         <img src=${value.robotSrc} alt=${value.robotNum}>`
        });
          $("#eventID").append(eventdata);
          $("#gamingeventID").append(gamingdata);
         //  $("#imageGallaryID").append(imageGallary);
          $("#notification").append(notification);
          $("#scheduleHeader").append(scheduleHeaders);
    ...
    $(document).ready(function() {
        ...
        let countImage = 0;
         let arr = document.querySelectorAll(".item-gallery");
         let leftBtn = document.querySelector(".left-gallery");
         let rightBtn = document.querySelector(".right-gallery");
         let max = arr.length;
         showImage(countImage);
         
         function showImage(index) {
           if (index < 0) countImage = max - 1;
         
           if (index >= max) countImage = 0;
         
           for (var i = 0; i < max; i++) {
             arr[i].style.display = "none";
           }
         
           arr[countImage].style.display = "block";
         }
